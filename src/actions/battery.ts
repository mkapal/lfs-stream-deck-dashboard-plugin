...
@action({ UUID: "com.martinkapal.lfs.dashboard.battery" })
export class BatteryAction extends SingletonAction {
  ...
  override async onWillAppear(ev: WillAppearEvent): Promise<void> {
    ...
    this.unsubscribe = outGaugeHub.subscribe((p: OutGaugePack) => {
      const isAvailable = (p.DashLights & DashLights.DL_BATTERY) !== 0;
      const isOn = (p.ShowLights & DashLights.DL_BATTERY) !== 0;
    ...
    streamDeck.logger.info(
      "BatteryAction appeared and subscribed to OutGaugeHub",
    );
  }
  ...
  override async onWillDisappear(_ev: WillDisappearEvent): Promise<void> {
    ...
    streamDeck.logger.info("BatteryAction disappeared and unsubscribed");
  }
}
